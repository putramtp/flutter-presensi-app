...
Berdasarkan permasalahan tersebut, dibuatlah aplikasi mobile dengan flutter ini agar dapat mengatasi permasalahan yang ada. Aplikasi ini di _compile_ pada android dan iOS sekaligus sehingga proses data yang berjalan dapat dilakukan pada device masing-masing dan tidak memberatkan server. Server hanya menerima kembalian data berupa _mapping JSON_ dari aplikasi ini dan memasukannya ke database utama tanpa harus melakukan serangkaian logic didalam server tersebut sekaligus. Adapun metode proses data yang digunakan adalah menggunakan REST API.
